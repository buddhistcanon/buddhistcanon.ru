...
class TextParser
{
    ...
    public function parse($text, $livewireId = null)
    {
        ...
        //$text = $this->processTermsVue($text, $livewireId);
        $text = $this->processTermsVue($text);
    ...
    public function processTermsLivewire($text, $livewireId)
    {
        ...
        foreach ($matches as $key => $match) {
            ...
            if (! $termVariant) {
                ...
                $text = str_replace(
                    $tag,
                    "<a href='".route('term.error', [$title])."' x-data @click.prevent=\"@this.call('clickTerm', '".$title."')\" class='text-red-500'>$title</a>",
                    $text
                );
    ...
    public function processTermsVue($text)
    {
        preg_match_all('/\[\[(.*?)\](.*?)\]/m', $text, $matches, PREG_SET_ORDER, 0);
        if (! $matches) {
            return $text;
