...
class ImportSuttaFromTheravadaRuService
{
    ...
    public function copyright()
    {
        ...
        if( ! $node) $node = $this->document->find("div[align=right]")[0]->first("span");
        if($node){
            $text = str_replace("\n", "", $node->innerHtml());
    ...
    public function content()
    {
        ...
        $contentArea = "error";
        if(isset($contentAreas[0]) AND strlen($contentAreas[0]->text()) > 200) $contentArea = $contentAreas[0];
        if(isset($contentAreas[1]) AND strlen($contentAreas[1]->text()) > 200) $contentArea = $contentAreas[1];
        if(isset($contentAreas[2]) AND strlen($contentAreas[2]->text()) > 200) $contentArea = $contentAreas[2];
        if(isset($contentAreas[3]) AND strlen($contentAreas[3]->text()) > 200) $contentArea = $contentAreas[3];
        if(isset($contentAreas[4]) AND strlen($contentAreas[4]->text()) > 200) $contentArea = $contentAreas[4];
        ...
        if(count($paragraphs) == 0) $paragraphs[0] = $paragraphs;
...
//                }
    ...
    {
        if($this->isNeedAttention) return 1;
        else return 0;
    }
