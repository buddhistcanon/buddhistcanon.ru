        ...
        <div class="d-flex flex-row">
            <div v-for="content in contents" class="me-4">
                <div class="form-check form-switch">
                    ...
                    <label class="form-check-label">{{content.lang}} <span v-if="content.lang !== 'pali'">({{content.translator.slug}})</span> #{{content.id}}</label>
                </div>
            </div>
        </div>
...
<!--                                <div v-show="!editedChunkId || editedChunkId !== chunk.id"  @click="selectChunkForEdit(chunk.content_id, chunk.id)" v-html="textToHtml(chunk.text)"></div>-->
                                <contenteditable tag="div" :contenteditable="true" v-model="chunk.text"></contenteditable>
                            </div>
                        </template>
                    </template>
                </div>
