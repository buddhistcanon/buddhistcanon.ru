...
import { Bars3Icon, BellIcon, XMarkIcon, ArrowUpIcon, ArrowDownIcon, ChevronDownIcon, MagnifyingGlassIcon } from '@heroicons/vue/24/outline'
        ...
        <Disclosure as="nav" class="bg-white" v-slot="{ open }">
                    ...
                    <div class="h-16 flex items-center">
                        ...
                        <div class="relative ml-8 flex-1 border-1 border-bc rounded-bc">
                            ...
                            <input type="text" name="search" id="search"
                                   class="block w-full rounded-bc bc-button-background border-0 py-2 pl-10 text-gray-900 ring-0 ring-inset ring-gray-bc placeholder:text-gray-400 focus:ring-2 focus:ring-offset-0 focus:ring-gray-200 text-sm" placeholder="Поиск" />
                        </div>
            ...
            <DisclosurePanel class="md:hidden">
                ...
                <div class="border-t border-gray-400 pt-4 pb-3">
                    <div v-if="!$page.props.auth.user">
                        <DisclosureButton :as="Link" :method="get" :href="'/login'" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:text-gray-700">Вход</DisclosureButton>
                    </div>
                    <div v-else>
                        ...
                        <div class="mt-3 space-y-1 px-2">
                            <DisclosureButton v-for="item in userNavigation" :key="item.name" :as="Link" :method="item.method" :href="item.href" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:text-gray-700">{{ item.name }}</DisclosureButton>
                        </div>
                    </div>
